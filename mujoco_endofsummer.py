...
# ==============================================================================
"""Example of using python simulate UI for a drone controller"""
...
blackbox_df = pd.read_csv('C:\\Users\\Akula\\Documents\\Summer2023_Stuff\\06_23_2023_Data\\drone_test_1_BB.csv', skiprows = lambda x: x in range(141)
...
gyro_x_bb = np.array(blackbox_df.loc[:, 'gyroADC[0]'].to_numpy(), ndmin = 2).T
gyro_y_bb = np.array(blackbox_df.loc[:, 'gyroADC[1]'].to_numpy(), ndmin = 2).T
gyro_z_bb = np.array(blackbox_df.loc[:, 'gyroADC[2]'].to_numpy(), ndmin = 2).T


bodyrates_groundt_full = np.hstack((gyro_x_bb, gyro_y_bb, gyro_z_bb))
...
if __name__ == '__main__':
  ...
  m = mujoco.MjModel.from_xml_path('C:\\Users\\Akula\\Documents\\Summer2023_Stuff\\Drone_Files\\hoopflyt_noground.xml')

  def foo(x, results=False, view=None):
  ...
  optimized_results = scipy.optimize.least_squares(lambda x: foo(x), x0, bounds=bounds, verbose=2, ftol=1e-2)#, diff_step=1.0)  
  ...
  optimal_results = foo(xstar, results=True, view=False)
  ...
  plt.legend()
  plt.subplot(312)
  plt.plot(optimal_results[:, 0], label='pitch_measured')
  plt.plot(bodyrates_groundt[:, 0], label='pitch_groundtruth')
  plt.plot(stickArray[:, 0]*xstar[9+0], label='pitch_input')
  plt.grid()
  plt.legend()
